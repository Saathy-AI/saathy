...
from src.saathy.streaming.models.events import EventType, SlackEvent
...
class TestEventManager:
    ...
    async def test_store_event(self, event_manager, mock_redis, sample_slack_event):
        ...
        mock_redis.setex.assert_any_call(
            event_key, 30 * 24 * 60 * 60, sample_slack_event.model_dump_json()
        )
    ...
    @pytest.mark.asyncio
    async def test_queue_for_correlation(
        self, event_manager, mock_redis, sample_slack_event
    ):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            try:
                await asyncio.wait_for(
                    event_manager.start_correlation_processor(), timeout=0.1
                )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_process_event(
        self, event_manager, mock_redis, sample_slack_event
    ):
