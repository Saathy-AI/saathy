...
from src.saathy.streaming.models.events import SlackEvent, GitHubEvent, EventType
...
class TestStreamingIntelligencePipeline:
    ...
    async def test_end_to_end_slack_github_correlation_and_action_generation(
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_for_integration):
            ...
            slack_event = SlackEvent(
                ...
                user_id="user123",
                platform="slack",
                raw_data={"channel": "C123", "text": "Urgent: need review of PR #456 - security vuln!"},
                channel_id="C123456",
            ...
            assert mock_redis_for_integration.setex.call_count >= 2  # Both events stored
            assert mock_redis_for_integration.lpush.call_count >= 2  # Both events queued
            ...
            correlation_calls = [call for call in mock_redis_for_integration.setex.call_args_list 
                               if "correlation:" in str(call[0][0])]
            assert len(correlation_calls) >= 1
            
            action_queue_calls = [call for call in mock_redis_for_integration.lpush.call_args_list 
            ...
            mock_redis_for_integration.get.return_value = json.dumps(mock_correlation_data)
            
            # Synthesize context
            context_bundle = await context_synthesizer.synthesize_context(correlation_id)
            ...
            action_store_calls = [call for call in mock_redis_for_integration.setex.call_args_list 
                                if "action:" in str(call[0][0])]
            assert len(action_store_calls) >= 1
            
            user_queue_calls = [call for call in mock_redis_for_integration.zadd.call_args_list 
    ...
    async def test_pipeline_with_insufficient_context(
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_for_integration):
            ...
            mock_openai_client.chat.completions.create.return_value = insufficient_response
            ...
            mock_redis_for_integration.get.return_value = json.dumps(weak_correlation_data)
    ...
    async def test_pipeline_with_daily_limit_exceeded(
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_for_integration):
            ...
            mock_redis_for_integration.get.return_value = json.dumps(good_correlation_data)
    ...
    async def test_error_handling_in_pipeline(self, mock_redis_for_integration):
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_for_integration):
            ...
            mock_redis_for_integration.zrangebyscore.side_effect = Exception("Correlation error")
