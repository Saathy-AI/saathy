...
class TestContextSynthesizer:
    ...
    @pytest.mark.asyncio
    async def test_synthesize_context_success(
        self, synthesizer, mock_redis, sample_correlation_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_synthesize_context_no_correlation_found(
        self, synthesizer, mock_redis
    ):
    ...
    def test_organize_by_platform(self, synthesizer, sample_correlation_data):
        ...
        assert (
            "Security Review Checklist" in platform_data["notion"]["pages"][0]["title"]
        )
    ...
    def test_extract_insights(self, synthesizer, sample_correlation_data):
        ...
        cross_platform_insight = next(
            (i for i in insights if "multiple platforms" in i), None
        )
    ...
    def test_extract_platform_specific_insights(
        self, synthesizer, sample_correlation_data
    ):
    ...
    def test_identify_urgency_signals(self, synthesizer, sample_correlation_data):
        ...
        urgency_signals = synthesizer.identify_urgency_signals(
        ...
        high_urgency_signal = next(
            (s for s in urgency_signals if "high-urgency events" in s), None
        )
        assert high_urgency_signal is not None

        # Should detect urgent keywords
        urgent_keywords_signal = next(
            (s for s in urgency_signals if "Urgent indicators" in s), None
        )
    ...
    def test_generate_context_narrative_slack_primary(
        self, synthesizer, sample_correlation_data
    ):
    ...
    def test_generate_platform_details(self, synthesizer):
        ...
        platform_data = {
            ...
            "notion": {"pages": [{"title": "Meeting Notes", "change_type": "created"}]},
        }

        details = synthesizer._generate_platform_details(
            "slack", platform_data["slack"]
        )
