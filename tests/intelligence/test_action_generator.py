...
def action_generator(mock_redis, mock_openai, mock_context_synthesizer):
    ...
    with patch("redis.asyncio.from_url", return_value=mock_redis):
        with patch(
            "src.saathy.intelligence.action_generator.ContextSynthesizer",
            return_value=mock_context_synthesizer,
        ):
            generator = ActionGenerator(
                openai_api_key="test_key", redis_url="redis://localhost:6379"
            )
...
class TestActionGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_actions_for_correlation_success(
        self,
        action_generator,
        ...
        with patch.object(action_generator, "check_daily_limits", return_value=True):
            ...
            with patch.object(
                action_generator, "validate_context_quality", return_value=True
            ):
                # Mock GPT-4 action generation
                with patch.object(
                    action_generator,
                    "generate_actions_with_gpt4",
                    return_value=sample_gpt_response["actions"],
                ):
                    ...
                    with patch.object(
                        action_generator,
                        "create_and_store_action",
                        return_value=mock_action,
                    ):
                        actions = (
                            await action_generator.generate_actions_for_correlation(
                                correlation_id
                            )
                        )
    ...
    @pytest.mark.asyncio
    async def test_generate_actions_no_context_bundle(
        self, action_generator, mock_context_synthesizer
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_context_quality_sufficient(
        self, action_generator, mock_openai, sample_context_bundle
    ):
        ...
        mock_response.choices[
            0
        ].message.content = '{"sufficient": true, "reasoning": "Good context"}'
    ...
    @pytest.mark.asyncio
    async def test_validate_context_quality_insufficient(
        self, action_generator, mock_openai, sample_context_bundle
    ):
        ...
        mock_response.choices[
            0
        ].message.content = '{"sufficient": false, "reasoning": "Weak context"}'
        mock_openai.chat.completions.create.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_generate_actions_with_gpt4(
        self, action_generator, mock_openai, sample_context_bundle, sample_gpt_response
    ):
        ...
        with patch.object(action_generator, "validate_action_data", return_value=True):
            with patch.object(
                action_generator, "enhance_action_links", side_effect=lambda x, y: x
            ):
                actions = await action_generator.generate_actions_with_gpt4(
                    sample_context_bundle
                )

                assert len(actions) == 1
                assert (
                    actions[0]["title"] == "Review PR #456 for security vulnerability"
                )
    ...
    def test_actions_seem_generic_true(self, action_generator):
        ...
        generic_actions = [
            {"title": "Check messages", "description": "Look at your messages"},
            {"title": "Review code", "description": "Check the code"},
        ]
    ...
    async def test_refine_generic_actions(self, action_generator, mock_openai):
        ...
        generic_actions = [
            {"title": "Check messages", "description": "Look at messages"}
        ]
        ...
        with patch.object(action_generator, "validate_action_data", return_value=True):
            refined_actions = await action_generator.refine_generic_actions(
                generic_actions, context
            )
    ...
    async def test_enhance_action_links(self, action_generator):
        ...
        context = {
            "platform_data": {
                "slack": {"messages": [{"channel": "eng-alerts"}]},
                "github": {
                ...
                "notion": {
                    "pages": [
                        {"title": "Security Review", "url": "https://notion.so/page123"}
                    ]
                },
            }
        }
        ...
        github_links = [
            link
            for link in enhanced_action["action_links"]
    ...
    async def test_create_and_store_action(self, action_generator, mock_redis):
        ...
        with patch.object(action_generator, "store_action") as mock_store:
            with patch.object(action_generator, "add_to_user_queue") as mock_queue:
                with patch.object(
                    action_generator, "increment_daily_counter"
                ) as mock_counter:
                    action = await action_generator.create_and_store_action(
                        action_data, correlation_id, user_id
                    )
    ...
    @pytest.mark.asyncio
    async def test_start_action_generation_processor(
        self, action_generator, mock_redis
    ):
        ...
        with patch.object(
            action_generator, "generate_actions_for_correlation", return_value=[]
        ):
            with patch.object(action_generator, "notify_user_of_actions"):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_generate_actions(
        self, action_generator, mock_context_synthesizer
    ):
        ...
        mock_context_synthesizer.synthesize_context.side_effect = Exception(
            "Context error"
        )
    ...
    async def test_close_connections(
        self, action_generator, mock_redis, mock_context_synthesizer
    ):
