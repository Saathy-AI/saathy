...
class TestEventCorrelator:
    ...
    @pytest.mark.asyncio
    async def test_process_event_correlation_with_related_events(
        self, correlator, mock_redis, sample_event_data
    ):
        ...
        with patch.object(
            correlator, "find_related_events", return_value=related_events
        ):
            with patch.object(
                correlator, "create_correlation_group", return_value="corr_123"
            ):
                with patch.object(
                    correlator, "trigger_action_generation"
                ) as mock_trigger:
    ...
    @pytest.mark.asyncio
    async def test_process_event_correlation_no_related_events(
        self, correlator, sample_event_data
    ):
    ...
    def test_calculate_event_type_correlation(self, correlator):
        ...
        score1 = correlator.calculate_event_type_correlation(
            {"event_type": "slack_message"}, {"event_type": "github_pr"}
        )
        assert score1 == 0.15

        # GitHub PR -> Slack reaction should get bonus
        score2 = correlator.calculate_event_type_correlation(
            {"event_type": "github_pr"}, {"event_type": "slack_reaction"}
        )
        assert score2 == 0.1

        # Unrelated types should get no bonus
        score3 = correlator.calculate_event_type_correlation(
            {"event_type": "slack_message"}, {"event_type": "notion_page_update"}
        )
        assert score3 == 0.1  # Some bonus for slack->notion

    @pytest.mark.asyncio
    async def test_create_correlation_group(
        self, correlator, mock_redis, sample_event_data, related_events_data
    ):
        """Test creating a correlation group."""
        correlation_id = await correlator.create_correlation_group(
            sample_event_data, related_events_data
        )
    ...
    def test_calculate_group_strength(
        self, correlator, sample_event_data, related_events_data
    ):
        ...
        strength = correlator.calculate_group_strength(
            sample_event_data, related_events_data
        )
        ...
        assert (
            strength > 0.5
        )  # Should be relatively strong due to good similarity scores
    ...
    async def test_trigger_action_generation(self, correlator, mock_redis):
        ...
        mock_redis.lpush.assert_called_once_with(
            "saathy:action_generation", correlation_id
        )
    ...
    async def test_update_correlation_status(self, correlator, mock_redis):
        ...
        existing_data = {"correlation_id": correlation_id, "status": "pending"}
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_process_event_correlation(
        self, correlator, sample_event_data
    ):
        """Test error handling in process_event_correlation."""
        # Make find_related_events fail
        with patch.object(
            correlator, "find_related_events", side_effect=Exception("Test error")
        ):
