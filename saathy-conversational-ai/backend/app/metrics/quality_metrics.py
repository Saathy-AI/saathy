...
class QualityMetrics:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.sufficiency_score = Summary(
            'saathy_sufficiency_score',
            'Context sufficiency scores'
        )
        
        self.expansion_rate = Gauge(
            'saathy_expansion_rate',
            'Rate of context expansion needed'
        )
        
        # Quality metrics
        self.relevance_score = Summary(
            'saathy_relevance_score',
            'Response relevance scores'
        )
        
        self.user_satisfaction = Summary(
            'saathy_user_satisfaction',
            'User satisfaction scores'
        )
        
        # System metrics
        self.cache_hit_rate = Gauge(
            'saathy_cache_hit_rate',
    ...
    async def track_user_feedback(self, session_id: str, feedback: Dict[str, Any]):
        ...
        satisfaction = feedback.get("relevance_score", 0.5) * 0.5 + \
    ...
    def _identify_quality_issues(self, metric_entry: Dict[str, Any]) -> List[str]:
        ...
        # Low sufficiency
        if metric_entry.get("sufficiency_score", 1.0) < self.thresholds["low_sufficiency"]:
    ...
    async def _queue_for_learning(self, metric_entry: Dict[str, Any], issues: List[str]):
    ...
    def get_session_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "satisfaction_scores": [t.get("satisfaction_score") for t in turns if "satisfaction_score" in t]
        }
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        sufficiency_scores = [t.get("sufficiency_score", 0) for t in all_turns if "sufficiency_score" in t]
        ...
        return {
            ...
            "avg_sufficiency_score": np.mean(sufficiency_scores) if sufficiency_scores else 0,
            "expansion_rate": sum(e > 0 for e in expansion_counts) / len(expansion_counts),
    ...
    async def export_metrics_for_analysis(self) -> Dict[str, Any]:
        ...
        for session_id, turns in self.conversation_metrics.items():
            ...
            if session_issues:
                problematic_conversations.append({
                    "session_id": session_id,
                    "problematic_turns": session_issues
                })
        ...
        return {
            ...
            "user_metrics_summary": {
                ...
                for user_id in list(self.user_metrics.keys())[:100]  # Limit to 100 users
            }
        }
