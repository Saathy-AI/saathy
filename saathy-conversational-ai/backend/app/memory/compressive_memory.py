...
class CompressiveMemoryManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.entity_importance_threshold = config.get("entity_importance_threshold", 0.6)
    ...
    async def compress_conversation(self, session_turns: List[Dict[str, Any]], 
        ...
        compressed_memory = {
            ...
            "metadata": {
                ...
                "compression_ratio": self._calculate_compression_ratio(session_turns, key_events),
                "compressed_at": datetime.utcnow().isoformat()
            }
        }
        
        logger.info(f"Compressed {len(session_turns)} turns to {len(key_events)} key events")
    ...
    async def _extract_user_profile(self, turns: List[Dict[str, Any]], 
                                  user_id: str) -> Dict[str, Any]:
        """Extract user preferences and patterns using GPT-4"""
        
        # Prepare conversation summary for analysis
        conversation_text = self._prepare_conversation_text(turns[:10])  # Analyze first 10 turns
        ...
        for i, turn in enumerate(turns):
            ...
            if importance_score > self.entity_importance_threshold:
                event = {
                    ...
                    "response_summary": self._summarize_response(turn.get("assistant_response", "")),
    ...
    def _track_entities(self, turns: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        entity_tracking = defaultdict(lambda: {
            "mentions": 0,
    ...
    def _identify_conversation_patterns(self, turns: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(turns) > 1:
            patterns["follow_up_rate"] = patterns["follow_up_frequency"] / (len(turns) - 1)
        
        return patterns
    
    def _calculate_turn_importance(self, turn: Dict[str, Any], index: int, 
                                 all_turns: List[Dict[str, Any]]) -> float:
    ...
    def _is_follow_up(self, prev_turn: Dict[str, Any], current_turn: Dict[str, Any]) -> bool:
        ...
        if prev_entities and current_entities:
            overlap = len(prev_entities.intersection(current_entities)) / len(prev_entities)
            if overlap > 0.5:
                return True
    ...
    def _calculate_compression_ratio(self, original_turns: List[Dict[str, Any]], 
    ...
    async def get_relevant_memory(self, query: str, information_needs: Dict[str, Any], 
                                compressed_memory: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for event in compressed_memory.get("key_events", []):
            ...
            # Check relevance
            if query_entities.intersection(event_entities) or \
               event.get("intent") == information_needs.get("intent"):
