...
class ContextRetrieverAgent:
    ...
    async def retrieve(self, information_needs: Dict[str, Any], user_id: str, 
                      expansion_hints: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if expansion_hints:
            info_needs_obj = self._apply_expansion_hints(info_needs_obj, expansion_hints)
    ...
    def _dict_to_info_needs(self, info_dict: Dict[str, Any], user_id: str) -> InformationNeeds:
    ...
    def _apply_expansion_hints(self, info_needs: InformationNeeds, 
                              expansion_hints: List[str]) -> InformationNeeds:
        ...
        for hint in expansion_hints:
            if hint == "temporal_coverage":
                ...
                if hasattr(info_needs.time_range, "start"):
                    info_needs.time_range["start"] = info_needs.time_range["start"] - timedelta(days=7)
    ...
    def _apply_rrf(self, retrieval_results: Dict[str, List[SearchResult]], 
                   information_needs: Dict[str, Any], user_id: str) -> List[SearchResult]:
        """
        ...
        logger.info(f"RRF fusion: {len(retrieval_results)} sources -> {len(fused_results)} unique results")
        
        return fused_results
    
    def _apply_relevance_boosting(self, results: List[SearchResult], 
                                 information_needs: Dict[str, Any]) -> List[SearchResult]:
        """
        ...
        for result in results:
            ...
            temporal_boost = self._calculate_temporal_boost(
                result.timestamp,
                information_needs.get("time_range", {})
            )
            ...
            entity_boost = self._calculate_entity_boost(
                result.content,
                result.metadata,
                information_needs.get("entities", {})
            )
    ...
    def _calculate_temporal_boost(self, item_timestamp: datetime, 
                                 query_time_context: Dict[str, Any]) -> float:
        """
    ...
    def _calculate_platform_boost(self, item_platform: str, 
                                 requested_platforms: List[str]) -> float:
        """
    ...
    def _calculate_entity_boost(self, content: str, metadata: Dict[str, Any], 
    ...
    def _group_by_platform(self, results: List[SearchResult]) -> Dict[str, List[SearchResult]]:
