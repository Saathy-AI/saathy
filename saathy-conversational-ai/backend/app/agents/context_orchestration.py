...
class ContextOrchestrationGraph:
    ...
    async def _retrieve_context(self, state: GraphState) -> GraphState:
        ...
        state["metadata"]["context_size"] = len(retrieved_context.get("all_results", []))
        
        logger.info(f"Retrieved {len(retrieved_context.get('all_results', []))} context items")
        return state
    ...
    async def _expand_context(self, state: GraphState) -> GraphState:
        ...
        state["metadata"][f"expansion_attempt_{state['expansion_attempts']}"] = expansion_plan
        
        logger.info(f"Expansion attempt {state['expansion_attempts']}: {expansion_plan.get('strategy', 'unknown')}")
        return state
    ...
    async def _generate_response(self, state: GraphState) -> GraphState:
        ...
        logger.info(f"Generated response with {len(state['context_used'])} context items")
        return state
    ...
    def _should_expand_context(self, state: GraphState) -> str:
        ...
        if sufficiency_score < 0.7 and expansion_attempts < self.max_expansion_attempts:
            logger.info(f"Expanding context (score: {sufficiency_score:.2f}, attempts: {expansion_attempts})")
            return "expand"
        else:
            logger.info(f"Proceeding to generation (score: {sufficiency_score:.2f}, attempts: {expansion_attempts})")
            return "generate"
    
    async def process_message(self, user_message: str, session_id: str, user_id: str, 
