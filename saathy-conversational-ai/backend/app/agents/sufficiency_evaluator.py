...
class SufficiencyEvaluatorAgent:
    ...
    async def evaluate(self, query: str, context: Dict[str, Any], 
                      information_needs: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        temporal_score = self._calculate_temporal_relevance(all_results, information_needs)
        platform_score = self._calculate_platform_coverage(all_results, information_needs)
        
        # GPT-4 completeness check (most important)
        gpt4_score = await self._gpt4_completeness_check(query, all_results, information_needs)
        ...
        logger.info(f"Sufficiency evaluation - Score: {overall_score:.2f}, Gaps: {gaps}")
    ...
    def _calculate_entity_coverage(self, results: List[Any], 
                                  information_needs: Dict[str, Any]) -> float:
        """
        ...
        coverage = len(covered_entities) / len(set(e.lower() for e in all_entities))
        
        logger.debug(f"Entity coverage: {len(covered_entities)}/{len(all_entities)} = {coverage:.2f}")
        
        return coverage
    
    def _calculate_temporal_relevance(self, results: List[Any], 
                                     information_needs: Dict[str, Any]) -> float:
        """
        ...
        logger.debug(f"Temporal relevance: {relevant_count}/{total_with_timestamp} = {relevance:.2f}")
        
        return relevance
    
    def _calculate_platform_coverage(self, results: List[Any], 
                                    information_needs: Dict[str, Any]) -> float:
        """
    ...
    async def _gpt4_completeness_check(self, query: str, results: List[Any], 
                                      information_needs: Dict[str, Any]) -> float:
        """
    ...
    def _prepare_context_summary(self, results: List[Any]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            summary_parts.append(
                f"{i}. {platform}{timestamp_str}: {content_preview}"
            )
        
        return "\n".join(summary_parts)
    
    def _identify_gaps(self, dimension_scores: Dict[str, float], 
        ...
        if information_needs.get("intent") == "query_actions" and not self._has_actions(results):
