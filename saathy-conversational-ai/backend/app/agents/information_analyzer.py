...
class InformationAnalyzerAgent:
    ...
    async def analyze(
        ...
        gpt_analysis = await self._gpt4_analyze(
            user_message, conversation_history, initial_intent, entities
        )
        ...
        information_needs = {
            ...
            "entities": {**entities, **gpt_analysis.get("entities", {})},
            "time_range": self._determine_time_range(
                time_refs, gpt_analysis.get("temporal_context")
            ),
        ...
        logger.info(
            f"Analyzed query - Intent: {information_needs['intent']}, "
    ...
    def _extract_entities(self, message: str) -> dict[str, list[str]]:
        ...
        entities = {"projects": [], "people": [], "features": [], "issues": []}
    ...
    def _extract_time_references(self, message: str) -> list[dict[str, Any]]:
        ...
        for pattern, delta in relative_patterns.items():
            if pattern in message_lower:
                time_refs.append(
                    {"type": "relative", "reference": pattern, "delta": delta}
                )
    ...
    async def _gpt4_analyze(
        self,
        message: str,
        conversation_history: list[dict[str, Any]],
        initial_intent: str,
        initial_entities: dict[str, list[str]],
    ) -> dict[str, Any]:
    ...
    def _determine_time_range(
        self, time_refs: list[dict[str, Any]], gpt_temporal: Optional[str]
    ) -> dict[str, Any]:
        ...
        if time_refs:
            ...
            return {"start": start_time, "end": end_time, "reference": ref["reference"]}
    ...
    def _extract_conversation_context(
        ...
        all_entities = {"projects": [], "people": [], "features": []}
