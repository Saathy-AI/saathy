...
from app.models.information_needs import (
    InformationNeeds, QueryIntent, TimeReference, 
    ExtractedEntity, QueryAnalysisResult
)
...
class BasicInformationAnalyzer:
    ...
    async def analyze_query(self, user_message: str, user_id: str, 
                          session_context: Optional[Dict] = None) -> QueryAnalysisResult:
        ...
        info_needs = InformationNeeds(
            ...
            previous_turn_relevant=self._check_turn_relevance(user_message, session_context)
        )
    ...
    def _extract_time_reference(self, query: str) -> Optional[TimeReference]:
        ...
        if date_match:
            ...
            return TimeReference(
                ...
                relative_expression=date_match.group(0)
            )
    ...
    def _assess_complexity(self, query: str, entities: List[ExtractedEntity], 
                          platforms: set) -> str:
    ...
    async def _gpt4_analysis(self, query: str, initial_intent: QueryIntent,
                           initial_entities: List[ExtractedEntity],
                           session_context: Optional[Dict]) -> Dict:
...
{{
    ...
    "time_context": "description of temporal context if any"
        ...
        try:
            response = await openai.ChatCompletion.acreate(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert at analyzing user queries."},
    ...
    def _determine_required_context(self, intent: QueryIntent, platforms: set) -> List[str]:
    ...
    def _check_turn_relevance(self, query: str, session_context: Optional[Dict]) -> bool:
