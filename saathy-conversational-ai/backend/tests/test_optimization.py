...
class TestContextCache:
    ...
    async def test_query_cache_miss(self, cache):
        ...
        result = await cache.get_cached_query_result("Unknown query", "user1")
    ...
    async def test_context_cache_fuzzy_matching(self, cache):
        ...
        await cache.cache_context(
            info_needs1, "user1", {"all_results": ["Result 1", "Result 2"]}
        )
        ...
        await cache.get_cached_context(info_needs2, "user1")
...
class TestQualityMetrics:
    ...
    async def test_track_user_feedback(self, metrics):
        ...
        # First track a turn
        await metrics.track_conversation_turn(
            "session1", {"query": "Test", "response_time": 1.0}
        )

        # Then add feedback
        feedback = {"relevance_score": 0.9, "completeness_score": 0.8, "helpful": True}
...
class TestLearningOptimizer:
    ...
    @pytest.fixture
    def optimizer(self):
        config = {"learning_rate": 0.1, "batch_size": 10}
    ...
    async def test_parameter_bounds(self, optimizer):
        ...
        feedback_items = [
            {"issues": ["low_sufficiency"], "metric_entry": {"sufficiency_score": 0.1}}
