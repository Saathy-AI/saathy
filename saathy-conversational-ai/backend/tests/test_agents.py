...
class TestContextOrchestrationGraph:
    ...
    async def test_process_message_flow(self, orchestration_graph):
        ...
        # Mock the agents
        orchestration_graph.information_analyzer.analyze = AsyncMock(
            return_value={
        ...
        orchestration_graph.context_retriever.retrieve = AsyncMock(
            return_value={
        ...
        orchestration_graph.sufficiency_evaluator.evaluate = AsyncMock(
            return_value={
                "score": 0.85,
                "gaps": [],
                "dimension_scores": {"entity_coverage": 0.9, "temporal_relevance": 0.8},
            }
        )

        orchestration_graph.response_generator.generate = AsyncMock(
            return_value={
    ...
    async def test_context_expansion_flow(self, orchestration_graph):
        ...
        orchestration_graph.context_expander.plan_expansion = AsyncMock(
            return_value={
        ...
        # Mock other agents
        orchestration_graph.information_analyzer.analyze = AsyncMock(
            return_value={"query": "Tell me about Project X", "intent": "get_context"}
        )

        orchestration_graph.context_retriever.retrieve = AsyncMock(
            return_value={"all_results": [Mock()], "metadata": {}}
        )

        orchestration_graph.response_generator.generate = AsyncMock(
            return_value={"response": "Project X details...", "context_used": []}
        )
...
class TestInformationAnalyzerAgent:
    ...
    def test_classify_intent_patterns(self, analyzer):
        ...
        assert (
            analyzer._classify_intent_patterns("What should I do today?")
            == "query_actions"
        )
        assert (
            analyzer._classify_intent_patterns("What happened with the bug?")
            == "query_events"
        )
        assert (
            analyzer._classify_intent_patterns("Show me the Dashboard project")
            == "get_context"
        )
        assert (
            analyzer._classify_intent_patterns("Why do you suggest this?")
            == "explain_action"
        )
    ...
    def test_extract_entities(self, analyzer):
        ...
        entities = analyzer._extract_entities(
            "Fix the Dashboard Project bug #123 with @john"
        )
    ...
    def test_extract_time_references(self, analyzer):
        ...
        refs = analyzer._extract_time_references(
...
class TestSufficiencyEvaluatorAgent:
    ...
    def test_calculate_entity_coverage(self, evaluator):
        ...
        info_needs = {
            "entities": {"projects": ["Dashboard", "Auth"], "issues": ["bug-123"]}
        }
    ...
    def test_calculate_temporal_relevance(self, evaluator):
        ...
        info_needs = {"time_range": {"start": now - timedelta(days=1), "end": now}}
        ...
        with patch.object(
            ...
            mock_response.choices = [
                Mock(
                    message=Mock(
                        content=json.dumps(
                            {"score": 0.75, "missing_elements": ["Recent updates"]}
                        )
                    )
                )
            ]
...
class TestEndToEndFlow:
    ...
    async def test_simple_query_flow(self):
        ...
        config = {"openai_api_key": "test-key", "max_expansion_attempts": 3}

        graph = ContextOrchestrationGraph(config)

        # Mock all external calls
        with patch.object(
            graph.information_analyzer.openai_client.chat.completions, "create"
        ):
            with patch.object(
                graph.context_retriever.basic_retriever, "retrieve_context"
            ):
                with patch.object(
                    graph.sufficiency_evaluator.openai_client.chat.completions, "create"
                ):
                    with patch.object(
                        graph.response_generator.openai_client.chat.completions,
                        "create",
                    ):
