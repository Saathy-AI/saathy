...
import pytest
from app.models.chat_session import ChatMessage, ChatSession, ConversationTurn
...
class TestAgenticChatServiceIntegration:
    ...
    async def test_complete_message_flow_with_caching(self, service, mock_db):
        ...
        with patch.object(
            service.orchestration_graph, "process_message"
        ) as mock_process:
    ...
    async def test_memory_compression_trigger(self, service, mock_db):
        ...
        with patch.object(
            service.memory_manager, "compress_conversation"
        ) as mock_compress:
    ...
    async def test_feedback_triggers_learning(self, service, mock_db):
        ...
        with patch.object(
            service.learning_optimizer, "process_feedback_batch"
        ) as mock_learn:
            mock_learn.return_value = {
                "status": "updated",
                "adjustments": {"sufficiency_threshold": -0.05},
                "new_params": {"sufficiency_threshold": 0.65},
            }
    ...
    async def test_cache_invalidation_on_new_data(self, service, mock_db):
        ...
        await service.context_cache.cache_query_result(
            "Old query", "test-user", {"response": "Old cached response"}
        )

        # Verify it's cached
        cached = await service.context_cache.get_cached_query_result(
            "Old query", "test-user"
        )
        ...
        cached_after = await service.context_cache.get_cached_query_result(
            "Old query", "test-user"
        )
