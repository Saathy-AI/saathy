...
class FrequencyController:
    ...
    async def can_send_notification(self, user_id: str) -> bool:
        ...
        try:
            ...
            if notification_count >= max_daily:
                logger.info(
                    f"User {user_id} has reached daily notification limit ({max_daily})"
                )
                return False
            ...
            if last_notification:
                ...
                if time_since_last < timedelta(minutes=15):
                    logger.info(
                        f"Rate limiting user {user_id}, last notification was {time_since_last.seconds/60:.1f} minutes ago"
                    )
                    return False
    ...
    async def get_user_notification_stats(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            last_notification = await self.redis.get(last_notification_key)
            if last_notification:
                stats["last_notification"] = datetime.fromisoformat(
                    last_notification.decode()
                )
    ...
    async def _get_user_preferences(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Return defaults
            return {"max_daily_notifications": 5, "batch_frequency": "hourly"}
