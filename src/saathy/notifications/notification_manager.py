...
class NotificationManager:
    ...
    async def notify_new_action(self, action_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            notification_priority, channels = await self.determine_notification_strategy(
                action_data, user_prefs
            )
            ...
            if should_notify_now:
                await self.send_immediate_notifications(action_data, channels, user_prefs)
            else:
                await self.queue_for_batch_notification(action_data, channels, user_prefs)
    ...
    async def determine_notification_strategy(self, 
                                            action_data: Dict[str, Any], 
                                            user_prefs: Dict[str, Any]) -> tuple[NotificationPriority, List[NotificationChannel]]:
        ...
        channels = user_prefs.get(f'{action_priority}_actions', [NotificationChannel.IN_APP])
    ...
    async def should_notify_immediately(self, 
                                      user_id: str,
                                      action_data: Dict[str, Any],
        ...
        if notification_priority == NotificationPriority.IMMEDIATE:
            ...
            if not await self.frequency_controller.can_send_notification(user_id):
                logger.info(f"Frequency limit reached for user {user_id}, batching instead")
                return False
    ...
    async def send_immediate_notifications(self, 
        ...
        for channel in channels:
            try:
                ...
                await self.track_notification_sent(user_id, action_data['action_id'], channel.value)
                
            except Exception as e:
                logger.error(f"Failed to send {channel.value} notification to {user_id}: {e}")

    async def queue_for_batch_notification(self, 
                                         action_data: Dict[str, Any], 
                                         channels: List[NotificationChannel],
                                         user_prefs: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Queued action {action_data['action_id']} for batch notification")
            
        except Exception as e:
            logger.error(f"Error queuing batch notification: {e}")

    async def prepare_notification_content(self, action_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    async def update_user_preferences(self, user_id: str, preferences: Dict[str, Any]) -> bool:
        ...
        try:
            prefs_key = f"user:{user_id}:notification_preferences"
            await self.redis.setex(prefs_key, 30 * 24 * 60 * 60, json.dumps(preferences))
            return True
        except Exception as e:
    ...
    async def send_browser_notification(self, user_id: str, content: Dict[str, str]):
        ...
        logger.info(f"Would send browser notification to {user_id}: {content['short_text']}")
    ...
    async def process_user_batch_notifications(self, user_id: str):
        ...
        try:
            ...
            logger.info(f"Processing {len(batch_items)} batch notifications for user {user_id}")
            ...
            logger.error(f"Error processing batch notifications for user {user_id}: {e}")