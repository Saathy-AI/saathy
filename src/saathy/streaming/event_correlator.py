...
class EventCorrelator:
    ...
    async def process_event_correlation(self, event_data: dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                f"Processing correlation for event {event_data['event_id']}, user {user_id}, timestamp {event_timestamp}"
            )
            ...
            if related_events:
                ...
                correlation_id = await self.create_correlation_group(
                    event_data, related_events
                )
                logger.info(
                    f"Created correlation group {correlation_id} with {len(related_events)} related events"
                )
    ...
    async def find_related_events(
        ...
        try:
            ...
            window_start = event_timestamp - timedelta(
                minutes=self.correlation_window_minutes
            )
            window_end = event_timestamp + timedelta(
                minutes=self.correlation_window_minutes
            )

            timeline_key = f"user:{user_id}:events"
            event_ids = await self.redis.zrangebyscore(
                timeline_key, window_start.timestamp(), window_end.timestamp()
            )
            ...
            for event_id in event_ids:
                ...
                if stored_event_data:
                    ...
                    similarity_score = self.calculate_event_similarity(
                        event_data, stored_event
                    )

                    if similarity_score >= self.similarity_threshold:
                        related_events.append(
                            {**stored_event, "similarity_score": similarity_score}
                        )
    ...
    def calculate_event_similarity(
        ...
        try:
            ...
            if (
                event1.get("project_context")
                and event2.get("project_context")
                and event1["project_context"] == event2["project_context"]
            ):
                score += 0.4
    ...
    def calculate_event_type_correlation(
        self, event1: dict[str, Any], event2: dict[str, Any]
    ) -> float:
    ...
    async def create_correlation_group(
        ...
        try:
            ...
            correlation_data = {
                ...
                "correlation_strength": self.calculate_group_strength(
            ...
            await self.redis.zadd(
                user_correlations_key, {correlation_id: datetime.now().timestamp()}
            )
    ...
    def calculate_group_strength(
        ...
        similarity_scores = [
            event.get("similarity_score", 0) for event in related_events
        ]
        ...
        platform_bonus = min(
            0.3, (len(platforms) - 1) * 0.1
        )  # Bonus for cross-platform

        # Bonus for urgency
        urgency_scores = [primary_event.get("urgency_score", 0)]
        urgency_scores.extend(
            [event.get("urgency_score", 0) for event in related_events]
        )
    ...
    async def get_user_correlations(
        ...
        try:
            ...
            correlation_ids = await self.redis.zrangebyscore(
                user_correlations_key, since_timestamp, datetime.now().timestamp()
            )

            correlations = []
            for correlation_id in correlation_ids:
                correlation_data = await self.get_correlation_by_id(
