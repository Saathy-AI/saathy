...
class EventManager:
    ...
    async def store_event(self, event: BaseEvent):
        ...
        try:
            ...
            await self.redis.zadd(
                user_timeline_key, {event.event_id: event.timestamp.timestamp()}
            )
            ...
            await self.redis.zadd(
                platform_index_key, {event.event_id: event.timestamp.timestamp()}
            )
    ...
    async def get_recent_events(
        ...
        try:
            ...
            event_ids = await self.redis.zrangebyscore(
                timeline_key, since_timestamp, datetime.now().timestamp()
            )
    ...
    async def get_platform_events(
        self, platform: str, hours: int = 24
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            event_ids = await self.redis.zrangebyscore(
                platform_index_key, since_timestamp, datetime.now().timestamp()
            )
        ...
        while True:
            try:
                ...
                if result:
                    ...
                    # Process correlation
                    if self.correlator:
                        await self.correlator.process_event_correlation(
                            correlation_data
                        )
