...
from .models.events import NotionEvent, EventType
...
class NotionPollingService:
    ...
    def __init__(self, notion_token: str, event_manager: EventManager, poll_interval: int = 300):
    ...
    async def start_polling(self):
        ...
        logger.info(f"Starting Notion polling service (interval: {self.polling_interval}s)...")
    ...
    async def check_database_changes(self, database_id: str):
        ...
        try:
            ...
            response = await self.client.databases.query(
                ...
                } if hasattr(self, 'last_check') else None,
                sorts=[{
                    "property": "last_edited_time", 
                    "direction": "descending"
                }],
    ...
    async def process_page_change(self, page_data: Dict[str, Any], database_id: Optional[str]):
        ...
        try:
            ...
            is_updated = (
                not is_new_page and 
                self.page_versions.get(page_id) != last_edited_time
            )
            ...
            properties_changed = self.detect_property_changes(page_id, page_details, is_new_page)
            
            # Skip if this looks like an automatic update (no significant changes)
            if not is_new_page and not properties_changed:
                logger.debug(f"Skipping page {page_id} - no significant changes detected")
                return
            ...
            keywords = self.extract_notion_keywords(page_title, page_details, properties_changed)
            project_context = self.infer_project_from_page(page_title, page_details, database_id)
            ...
            notion_event = NotionEvent(
                ...
                timestamp=datetime.fromisoformat(last_edited_time.replace("Z", "+00:00")),
    ...
    def extract_title(self, data: Dict[str, Any]) -> str:
        ...
        for prop_name, prop_data in title_properties.items():
    ...
    def detect_property_changes(self, page_id: str, page_data: Dict[str, Any], is_new: bool) -> List[str]:
        ...
        significant_props = ["Status", "Assignee", "Priority", "Due Date", "Tags", "Project"]
        
        for prop_name, prop_data in properties.items():
            if any(sig_prop.lower() in prop_name.lower() for sig_prop in significant_props):
    ...
    def extract_notion_keywords(self, title: str, page_data: Dict[str, Any], changes: List[str]) -> List[str]:
        ...
        notion_keywords = [
            'project', 'task', 'bug', 'feature', 'meeting', 'notes', 'spec', 'design',
            'deadline', 'milestone', 'release', 'sprint', 'epic', 'todo', 'done',
            'review', 'feedback', 'documentation', 'docs', 'requirements'
        ]
    ...
    def infer_project_from_page(self, title: str, page_data: Dict[str, Any], database_id: Optional[str]) -> Optional[str]:
    ...
    def calculate_notion_urgency(self, title: str, properties_changed: List[str], 
                                keywords: List[str], is_new: bool) -> float:
        """Calculate urgency for Notion changes."""
        score = 0.0
        title_lower = title.lower()
        
        # Urgent keywords in title
        if any(word in title_lower for word in ['urgent', 'critical', 'asap', 'deadline', 'emergency']):
            score += 0.4
        ...
        status_changes = [p for p in properties_changed if 'status' in p.lower() or 'priority' in p.lower()]
        if status_changes:
            score += 0.3
