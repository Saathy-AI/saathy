...
from .models.events import GitHubEvent, EventType
...
class GitHubWebhookProcessor:
    ...
    async def process_webhook(self, payload: Dict[str, Any], event_type: str, delivery_id: str) -> Dict[str, str]:
    ...
    async def handle_push_event(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            for commit in commits:
                ...
                keywords = self.extract_commit_keywords(commit_message, files_changed, branch)
                urgency_score = self.calculate_commit_urgency(commit_message, files_changed, branch)
                ...
                logger.debug(f"Processed push event for commit {commit_sha[:8]} in {repository}")
    ...
    async def handle_pr_event(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            await self.event_manager.process_event(github_event)
            logger.debug(f"Processed PR {action} event for #{pr_number} in {repository}")
    ...
    async def handle_issue_event(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            await self.event_manager.process_event(github_event)
            logger.debug(f"Processed issue {action} event for #{issue_number} in {repository}")
    ...
    async def handle_pr_review_event(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            keywords = ["review", review_state.lower()] + self.extract_github_mentions(review_body)
            ...
            await self.event_manager.process_event(github_event)
            logger.debug(f"Processed review {action} event for PR #{pr_number} in {repository}")
    ...
    def extract_commit_keywords(self, message: str, files: List[str], branch: str) -> List[str]:
        """Extract keywords from commit message and context."""
        keywords = []
        message_lower = message.lower()
        
        # Conventional commit prefixes
        commit_keywords = ['fix', 'feat', 'docs', 'style', 'refactor', 'test', 'chore',
                          'bug', 'hotfix', 'release', 'merge', 'revert', 'perf', 'build']
        ...
        for file_path in files:
            ...
            if any(ext in file_lower for ext in ['.py', '.js', '.ts', '.go', '.java', '.rb']):
                keywords.append('code')
            elif any(ext in file_lower for ext in ['.md', '.txt', '.rst', '.adoc']):
                keywords.append('docs')
            elif any(ext in file_lower for ext in ['.json', '.yaml', '.yml', '.toml', '.ini']):
                keywords.append('config')
            elif any(ext in file_lower for ext in ['.sql', '.db']):
                keywords.append('database')
            elif any(name in file_lower for name in ['dockerfile', 'docker-compose', '.docker']):
    ...
    def extract_pr_keywords(self, text: str, action: str) -> List[str]:
        ...
        pr_keywords = ['feature', 'bug', 'fix', 'docs', 'refactor', 'test', 'security',
                      'performance', 'ui', 'api', 'database', 'migration', 'breaking',
                      'wip', 'draft', 'ready', 'urgent']
    ...
    def extract_issue_keywords(self, text: str, action: str, issue_data: Dict[str, Any]) -> List[str]:
        ...
        # Issue type keywords
        issue_keywords = ['bug', 'feature', 'enhancement', 'question', 'help', 'documentation',
                         'good first issue', 'urgent', 'critical', 'blocked']
    ...
    def extract_comment_keywords(self, text: str, is_pr: bool) -> List[str]:
        ...
        review_keywords = ['lgtm', 'looks good', 'approved', 'changes requested', 
                          'needs work', 'question', 'suggestion', 'nit']
    ...
    def calculate_commit_urgency(self, message: str, files: List[str], branch: str) -> float:
        """Calculate urgency for commits."""
        score = 0.0
        message_lower = message.lower()
        
        # Urgent commit keywords
        if any(word in message_lower for word in ['hotfix', 'urgent', 'critical', 'security', 'emergency']):
            score += 0.5
    ...
    def calculate_pr_urgency(self, text: str, action: str, pr_data: Dict[str, Any]) -> float:
        ...
        if any(word in text_lower for word in ['urgent', 'hotfix', 'security', 'critical', 'emergency']):
            score += 0.4
    ...
    def calculate_issue_urgency(self, text: str, action: str, issue_data: Dict[str, Any]) -> float:
        ...
        # Check for urgent keywords
        if any(word in text_lower for word in ['urgent', 'critical', 'emergency', 'blocking', 'broken']):
            score += 0.4
    ...
    def calculate_comment_urgency(self, text: str, is_pr: bool) -> float:
        ...
        if is_pr:
            if any(phrase in text_lower for phrase in ['changes requested', 'needs work', 'blocking']):
                score += 0.3
            elif any(phrase in text_lower for phrase in ['lgtm', 'approved', 'looks good']):
                score += 0.2
