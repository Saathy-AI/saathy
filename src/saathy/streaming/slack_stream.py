...
from .models.events import SlackEvent, EventType
...
class SlackStreamProcessor:
    ...
    async def populate_caches(self):
        ...
        try:
            ...
            logger.info(f"Cached {len(self.user_cache)} users and {len(self.channel_cache)} channels")
            
        except Exception as e:
            logger.error(f"Error populating caches: {e}")
        
    async def handle_events(self, client: AsyncSocketModeClient, req: SocketModeRequest):
    ...
    async def handle_message_event(self, event_data: Dict[str, Any]):
        ...
        if (event_data.get("bot_id") or 
            event_data.get("hidden") or 
            event_data.get("subtype") in ["message_changed", "message_deleted"]):
            return
        ...
        try:
            ...
            if not channel_info:
                ...
                try:
                    channel_response = await self.web_client.conversations_info(channel=channel_id)
                    if channel_response["ok"]:
                        channel_info = {
                            "name": channel_response["channel"].get("name", "unknown"),
                            "is_private": channel_response["channel"].get("is_private", False)
                        }
            ...
            urgency_score = self.calculate_urgency(message_text, mentioned_users, channel_name)
    ...
    async def handle_reaction_event(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(f"Processed Slack reaction {reaction} from {user_id} in #{channel_name}")
    ...
    def extract_keywords(self, text: str) -> List[str]:
        ...
        tech_keywords = [
            'bug', 'error', 'deploy', 'deployment', 'release', 'urgent', 'fix', 'issue', 
            'pr', 'pull request', 'merge', 'review', 'test', 'testing', 'production', 
            'staging', 'api', 'database', 'server', 'down', 'outage', 'incident',
            'meeting', 'call', 'sync', 'standup', 'demo', 'presentation',
            'deadline', 'due', 'milestone', 'launch', 'ship', 'rollout'
        ]
    ...
    def calculate_urgency(self, text: str, mentioned_users: List[str], channel_name: str) -> float:
        ...
        if any(word in text_lower for word in ['production', 'prod', 'outage', 'down', 'incident']):
            score += 0.3
        ...
        if any(word in text_lower for word in ['deadline', 'today', 'now', 'quick', 'fast']):
            score += 0.2
        
        # Channel-based urgency
        if any(word in channel_name.lower() for word in ['alert', 'incident', 'urgent', 'critical']):
            score += 0.2
    ...
    def infer_project_context(self, channel_name: str, message_text: str) -> Optional[str]:
        ...
        if channel_name and channel_name != "unknown":
            ...
            if clean_name.lower() not in ['general', 'random', 'water cooler', 'announcements']:
                return channel_name
    ...
    async def send_message(self, channel_id: str, text: str, thread_ts: Optional[str] = None):
