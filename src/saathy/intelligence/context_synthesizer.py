...
class ContextSynthesizer:
    ...
    async def synthesize_context(self, correlation_id: str) -> Optional[ContextBundle]:
        ...
        try:
            ...
            logger.info(
                f"Synthesizing context for correlation {correlation_id} with {len(related_events)} related events"
            )
            ...
            key_insights = self.extract_insights(
                primary_event, related_events, platform_data
            )

            # Identify urgency signals
            urgency_signals = self.identify_urgency_signals(
                primary_event, related_events
            )
            ...
            logger.debug(
                f"Successfully synthesized context for correlation {correlation_id}"
            )
            return context_bundle

        except Exception as e:
            logger.error(
                f"Error synthesizing context for correlation {correlation_id}: {e}"
            )
            return None

    def organize_by_platform(
        self, primary_event: dict[str, Any], related_events: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def extract_insights(
        ...
        platforms = {event["platform"] for event in all_events}
        if len(platforms) > 1:
        ...
        projects = {
            event.get("project_context")
            for event in all_events
            if event.get("project_context")
        }
        if projects:
        ...
        timestamps = [
            datetime.fromisoformat(event["timestamp"]) for event in all_events
        ]
        if timestamps:
            ...
            if time_span.total_seconds() < 300:  # 5 minutes
                ...
                insights.append(
                    "Events occurred within a short time window (< 30 minutes)"
                )
        ...
        users = {event["user_id"] for event in all_events}
        if len(users) > 1:
            insights.append(f"Multiple users involved: {', '.join(list(users)[:3])}")

        return insights[:10]  # Limit to top 10 insights

    def _extract_platform_specific_insights(
        ...
        if slack_data.get("messages"):
            ...
            if message_count > 3:
                insights.append(
                    f"Active Slack discussion with {message_count} messages"
                )

            channels = slack_data.get("channels", [])
            if len(channels) > 1:
                insights.append(
                    f"Discussion spans multiple Slack channels: {', '.join(channels[:3])}"
                )
    ...
    def identify_urgency_signals(
        ...
        pr_events = [
            e
            for e in all_events
            if e.get("platform") == "github" and "pr" in e.get("keywords", [])
        ]
        if pr_events:
        ...
        total_mentions = sum(
            len(event.get("mentioned_users", [])) for event in all_events
        )
        if total_mentions > 0:
            ...
            if primary_platform == "slack":
                ...
                if primary_event.get("pr_number"):
                    narrative_parts.append(
                        f"Started with GitHub PR #{primary_event['pr_number']} {action} in {repo}"
                    )
            ...
            if related_events:
                ...
                if len(platforms_mentioned) == 1:
                    ...
                    activity_summary.append(
                        f"{len(related_events)} related {platform_name} events"
                    )
                else:
                    activity_summary.append(
                        f"related activity across {', '.join(platforms_mentioned)}"
                    )
    ...
    def _generate_platform_details(self, platform_data: dict[str, Any]) -> list[str]:
        ...
        if github_data.get("commits"):
            ...
            if len(commits) == 1:
                commit = commits[0]
                details.append(
                    f"Git commit {commit['sha']} on {commit.get('branch', 'unknown')}"
                )
            elif len(commits) > 1:
    ...
    async def store_context_bundle(self, context_bundle: ContextBundle):
        ...
        try:
            ...
            logger.debug(
                f"Stored context bundle for correlation {context_bundle.correlation_id}"
            )
