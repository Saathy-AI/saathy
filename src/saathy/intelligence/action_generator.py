...
from .models.actions import GeneratedAction, ActionPriority, ActionType, ActionLink
...
class ActionGenerator:
    ...
    async def generate_actions_for_correlation(self, correlation_id: str) -> List[GeneratedAction]:
        ...
        try:
            ...
            context_bundle = await self.context_synthesizer.synthesize_context(correlation_id)
            if not context_bundle:
                logger.error(f"No context bundle found for correlation {correlation_id}")
                return []
            
            # Check daily limits for user
            if not await self.check_daily_limits(context_bundle.user_id):
                logger.info(f"Daily action limit reached for user {context_bundle.user_id}")
                return []
            
            # Validate context quality
            if not await self.validate_context_quality(context_bundle):
                logger.info(f"Context quality insufficient for correlation {correlation_id}")
                return []
            ...
            logger.info(f"Generated {len(stored_actions)} actions for correlation {correlation_id}")
            return stored_actions
            
        except Exception as e:
            logger.error(f"Error generating actions for correlation {correlation_id}: {e}")
            return []
    ...
    async def check_daily_limits(self, user_id: str) -> bool:
        ...
        try:
            ...
            today_key = f"user:{user_id}:actions:daily:{datetime.now().strftime('%Y-%m-%d')}"
    ...
    async def validate_context_quality(self, context_bundle) -> bool:
        ...
        try:
            ...
            if context_bundle.correlation_strength < 0.3:
                logger.debug(f"Correlation strength too low: {context_bundle.correlation_strength}")
                return False
            
            # Use GPT-4 for sophisticated context validation
            validation_prompt = get_context_validation_prompt(context_bundle.model_dump())
            ...
            logger.debug(f"Context validation for {context_bundle.correlation_id}: {is_sufficient} - {reasoning}")
    ...
    async def generate_actions_with_gpt4(self, context_bundle) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if validated_actions and self.actions_seem_generic(validated_actions):
                ...
                validated_actions = await self.refine_generic_actions(validated_actions, context_data)
    ...
    def validate_action_data(self, action: Dict[str, Any]) -> bool:
        ...
        required_fields = ['title', 'description', 'priority', 'action_type', 'reasoning']
        ...
        valid_types = ['review', 'respond', 'update', 'meeting', 'follow_up', 'create', 'fix']
        ...
        generic_indicators = ['check', 'look at', 'review the', 'update the', 'follow up on the']
        if any(indicator in title or indicator in description for indicator in generic_indicators):
    ...
    def actions_seem_generic(self, actions: List[Dict[str, Any]]) -> bool:
        ...
        generic_keywords = ['check', 'review', 'update', 'follow up', 'look at', 'consider']
        ...
        for action in actions:
            ...
            if len(title.split()) <= 3 and any(word in title for word in generic_keywords):
                return True
                
            # If description doesn't contain specific references
            if not any(indicator in description for indicator in ['#', 'pr', 'issue', '@', 'commit']):
                if any(word in description for word in generic_keywords):
                    return True
                    
        return False
    
    async def refine_generic_actions(self, actions: List[Dict[str, Any]], context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def enhance_action_links(self, action: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if slack_data.get('messages'):
                for message in slack_data['messages'][-2:]:  # Last 2 messages
                    ...
                    slack_url = f"slack://channel?team=T123456&id=C{hash(channel) % 1000000}"
    ...
    async def create_and_store_action(self, action_data: Dict[str, Any], correlation_id: str, user_id: str) -> Optional[GeneratedAction]:
        ...
        try:
            ...
            if priority == 'urgent':
                expires_at = datetime.now() + timedelta(hours=4)  # Urgent actions expire quickly
            elif priority == 'high':
                expires_at = datetime.now() + timedelta(hours=12)
            elif priority in ['low', 'fyi']:
                expires_at = datetime.now() + timedelta(days=3)  # Low priority actions last longer
            ...
            generated_action = GeneratedAction(
                ...
                context_summary=action_data.get('context_summary', action_data['description'][:200]),
            ...
            logger.info(f"Created and stored action {action_id}: {generated_action.title}")
    ...
    async def add_to_user_queue(self, user_id: str, action_id: str):
        ...
        try:
            ...
            await self.redis.zadd(user_queue_key, {action_id: datetime.now().timestamp()})
    ...
    async def start_action_generation_processor(self):
        ...
        while True:
            try:
                if not self.redis:
                    logger.error("Redis not initialized for action generation processor")
                ...
                if result:
                    ...
                    logger.info(f"Processing action generation for correlation {correlation_id}")
                    
                    # Generate actions
                    actions = await self.generate_actions_for_correlation(correlation_id)
                    
                    if actions:
                        logger.info(f"Generated {len(actions)} actions for correlation {correlation_id}")
                        
                        # Notify users about new actions (could integrate with Slack/email here)
                        await self.notify_user_of_actions(actions)
                    else:
                        logger.warning(f"No actions generated for correlation {correlation_id}")
    ...
    async def get_user_actions(self, user_id: str, limit: int = 20) -> List[GeneratedAction]:
