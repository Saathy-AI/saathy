...
class GeneratedAction(BaseModel):
    ...
    estimated_time_minutes: int = Field(
        default=15, description="Estimated time to complete"
    )

    # Links and actions
    action_links: list[ActionLink] = Field(
        default=[], description="Links to take action"
    )
    related_people: list[str] = Field(
        default=[], description="People involved or to notify"
    )

    # Metadata
    user_id: str = Field(..., description="User this action is for")
    correlation_id: str = Field(
        ..., description="Event correlation that generated this"
    )
    generated_at: datetime = Field(default_factory=datetime.now)
    expires_at: Optional[datetime] = Field(
        None, description="When action becomes irrelevant"
    )
    ...
    user_feedback: Optional[str] = Field(
        None, description="User feedback on usefulness"
    )
    feedback_score: Optional[float] = Field(
        None, description="0-1 score from user feedback"
    )
...
class ContextBundle(BaseModel):
    ...
    correlation_strength: float = Field(
        default=0.0, description="0-1 strength of correlation"
    )
