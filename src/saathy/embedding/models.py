...
class OpenAIModel(EmbeddingModel):
    ...
    def __init__(self, metadata: ModelMetadata, api_key: str, model_name: str = "text-embedding-ada-002"):
        super().__init__(metadata)
...
class ModelRegistry:
    ...
    def get_best_model_for_content(self, content_type: str, quality: str = "balanced") -> Optional[EmbeddingModel]:
        ...
        for model in self._models.values():
            if content_type == "code" and model.metadata.code_specialized:
                ...
                candidates.append(model)
            elif content_type == "multimodal" and model.metadata.model_type == "multimodal":
                candidates.append(model)
        ...
        if quality == "fast":
            candidates.sort(key=lambda m: (-m.metadata.performance_score, m.metadata.dimensions))
        elif quality == "high":
            candidates.sort(key=lambda m: (m.metadata.performance_score, -m.metadata.dimensions))
        else:  # balanced
