...
class CodeChunker(ChunkingStrategy):
    ...
    def _chunk_structure(
        self, structure: dict[str, Any], metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
...
class DocumentChunker(ChunkingStrategy):
        ...
        if len(content) <= self.max_chunk_size:
            return [
                Chunk(
                    ...
                    chunk_type="document",
                    metadata=metadata
                    or ChunkMetadata(content_type=ContentType.DOCUMENT),
                )
            ]
    ...
    def _chunk_section(
        self, section: dict[str, Any], metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
...
class MeetingChunker(ChunkingStrategy):
        ...
        if len(content) <= self.max_chunk_size:
            return [
                Chunk(
                    ...
                    chunk_type="meeting",
                    metadata=metadata
                    or ChunkMetadata(content_type=ContentType.MEETING),
                )
            ]
    ...
    def _chunk_speaker_turn(
        self, turn: dict[str, Any], metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
...
class GitCommitChunker(ChunkingStrategy):
    ...
    def _chunk_commit_part(
        self, part: dict[str, Any], metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
        ...
        if len(content) <= self.max_chunk_size:
            chunk_metadata = metadata or ChunkMetadata(
                content_type=ContentType.GIT_COMMIT
            )
            chunk_metadata.custom_fields.update(
                {
...
class SlackMessageChunker(ChunkingStrategy):
    ...
    def _chunk_message(
        self, message: dict[str, Any], metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
