"""Main chunking processor with automatic strategy selection."""

import logging
from typing import Any, Optional
from pathlib import Path

from .core import ChunkingProcessor as ChunkingProcessorInterface, ChunkingConfig, ChunkMetadata, ContentType
from .core.exceptions import StrategyNotFoundError, ChunkingConfigurationError
from .strategies import (
    FixedSizeChunker, SemanticChunker, CodeChunker, DocumentChunker,
    MeetingChunker, GitCommitChunker, SlackMessageChunker, EmailChunker
)
from .utils import ContentTypeDetector, ChunkQualityValidator, ChunkMerger, ChunkCache, generate_content_hash
...
class ChunkingProcessor(ChunkingProcessorInterface):
    ...
    def __init__(self, config: Optional[ChunkingConfig] = None):
        ...
        self.cache = ChunkCache(self.config.cache_ttl) if self.config.enable_caching else None
    ...
    def chunk_content(self, 
        ...
        if content_type is None:
            detected_type = self.content_detector.detect_content_type(content, file_extension)
        ...
        if not strategy:
            logger.warning(f"No strategy found for content type: {content_type_enum.value}")
        ...
        if not quality_metrics["valid"]:
            logger.warning(f"Chunking quality issues detected: {quality_metrics['quality_issues']}")
    ...
    def chunk_file(self, file_path: str, metadata: Optional[dict[str, Any]] = None) -> list[Chunk]:
        ...
        try:
            ...
            return self.chunk_content(content, file_extension=file_extension, metadata=file_metadata)
    ...
    def get_strategy(self, content_type: ContentType) -> Any:
        ...
        if not strategy:
            raise StrategyNotFoundError(f"No strategy found for content type: {content_type.value}")
        return strategy
