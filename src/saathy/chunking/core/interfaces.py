...
class ChunkingConfig:
    """Configuration for chunking processor."""
    ...
    def validate(self) -> None:
        ...
        if self.overlap >= self.max_chunk_size:
            raise ValueError("overlap cannot be greater than or equal to max_chunk_size")
...
class ChunkingStrategy(ABC):
    ...
    def _validate_chunk(self, chunk: Chunk) -> bool:
        ...
        return (len(chunk.content) >= self.min_chunk_size and 
                len(chunk.content) <= self.max_chunk_size)


class ChunkingProcessor(ABC):
    """Abstract base class for chunking processors."""
...
class ContentTypeDetector(ABC):
    """Abstract base class for content type detection."""
...
class ChunkQualityValidator(ABC):
    """Abstract base class for chunk quality validation."""
...
class ChunkMerger(ABC):
    """Abstract base class for chunk merging."""
...
class ChunkCache(ABC):
    """Abstract base class for chunk caching."""
