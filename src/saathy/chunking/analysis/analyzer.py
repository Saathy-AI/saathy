...
class ChunkAnalyzer:
    ...
    def analyze_chunks(self, chunks: list[Chunk], original_content: str) -> ChunkQualityMetrics:
        ...
        coverage_ratio = total_chunk_length / len(original_content) if original_content else 0.0
    ...
    def _calculate_overlap_efficiency(self, chunks: list[Chunk]) -> float:
        ...
        for i in range(len(chunks) - 1):
            ...
            expected_overlap = min(len(current_chunk.content), len(next_chunk.content)) * 0.1  # 10% expected
    ...
    def get_chunk_statistics(self, chunks: list[Chunk]) -> dict[str, Any]:
        ...
        return {
            ...
            "overlap_stats": {
                "avg_previous_overlap": sum(chunk.overlap_with_previous for chunk in chunks) / len(chunks),
                "avg_next_overlap": sum(chunk.overlap_with_next for chunk in chunks) / len(chunks)
            }
        } 