...
class EmailChunker(BaseChunkingStrategy):
    ...
    def _chunk_headers(
        self, headers: str, metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
        """Chunk email headers."""
    ...
    def _chunk_body(
        self, body: str, metadata: Optional[ChunkMetadata] = None
    ) -> list[Chunk]:
    ...
    def _group_headers(self, headers: str) -> list[str]:
        ...
        for line in lines:
            ...
            is_new_header = any(
                re.match(pattern, line) for pattern in self.header_patterns
            )
