...
class UserPreferencesAPI:
    ...
    async def update_user_preferences(self, user_id: str, preferences: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Save to Redis
            await self.redis.setex(prefs_key, 30 * 24 * 60 * 60, json.dumps(updated_prefs))
    ...
    def _validate_preferences(self, preferences: Dict[str, Any]) -> bool:
        ...
        if "notifications" in preferences:
            ...
            for action_type in ["urgent_actions", "high_actions", "medium_actions", "low_actions", "fyi_actions"]:
            ...
            if "batch_frequency" in notif and notif["batch_frequency"] not in valid_frequencies:
                return False
            
            # Check max daily notifications
            if "max_daily_notifications" in notif:
                if not isinstance(notif["max_daily_notifications"], int) or notif["max_daily_notifications"] < 0:
                    return False
        ...
        if "dashboard" in preferences:
            ...
            if "items_per_page" in dash:
                if not isinstance(dash["items_per_page"], int) or dash["items_per_page"] < 1:
                    return False
