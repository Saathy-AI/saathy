...
from ...intelligence.models.actions import GeneratedAction, ActionPriority
...
class ActionAPI:
        ...
        try:
            ...
            for action_id in action_ids:
                ...
                if action_data:
                    ...
                    # Apply filters
                    if priority_filter and action.get('priority') not in priority_filter:
                        continue
    ...
    async def update_action_status(self, 
                                 user_id: str, 
                                 action_id: str, 
                                 status: str, 
    ...
    async def get_action_details(self, user_id: str, action_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def track_action_event(self, user_id: str, action_id: str, event_type: str, metadata: Optional[str] = None):
...
async def get_actions(
    ...
    priority: Optional[str] = Query(None, description="Priority filter: urgent,high,medium,low,fyi"),
    status: Optional[str] = Query(None, description="Status filter: pending,completed,dismissed"),
...
async def update_action_status(
    ...
    try:
        success = await action_api.update_action_status(user_id, action_id, status, feedback)
...
async def get_action_detail(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve action details")
